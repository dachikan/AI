    ...
    if (filter_var($noteUrl, FILTER_VALIDATE_URL) && strpos($noteUrl, 'note.com') !== false) {
        try {
            ...
            if (count($existingRegistrations) > 0) {
                ...
                // 最新取得したタイトルを設定
                $_POST['fetched_title'] = $noteTitle;
            } else {
                $urlCheckResult = 'new';
                $message = '未登録の記事です。新規登録できます。';
                // タイトルをPOSTデータに設定
                $_POST['note_title'] = $noteTitle;
            }
        } catch (Exception $e) {
...
if (isset($_POST['register_article'])) {
    ...
    if (empty($noteUrl) || empty($noteUsername) || empty($aiServiceId)) {
        ...
        try {
            ...
            // ユーザーを取得または作成（既存のカラムのみ使用）
            if (!empty($email)) {
                // メールアドレスがある場合
                $stmt = $conn->prepare("SELECT id FROM ai_users WHERE note_username = ? OR email = ?");
            ...
            if ($result->fetch_assoc()) {
                ...
                if (!empty($email)) {
                    ...
                    $message = '記事が正常に登録されました！（メールアドレス未入力のため認証機能は利用できません）';
                }
...
// タイトル更新処理
if (isset($_POST['update_title']) && !empty($_POST['note_url']) && !empty($_POST['new_title'])) {
    $noteUrl = trim($_POST['note_url']);
    $newTitle = trim($_POST['new_title']);
    
    try {
        // 該当URLの全記事のタイトルを更新
        $stmt = $conn->prepare("UPDATE ai_articles SET title = ? WHERE url = ?");
        $stmt->bind_param("ss", $newTitle, $noteUrl);
        $stmt->execute();
        
        if ($stmt->affected_rows > 0) {
            $message = 'タイトルが正常に更新されました！';
            
            // 更新後も同じ画面を維持するため、URL確認状態を再現
            $_POST['check_url'] = 1;
            $urlCheckResult = 'exists';
            ...
            // 更新されたタイトルを表示用に設定
            $_POST['fetched_title'] = $newTitle;
            
        } else {
            $error = 'タイトルの更新に失敗しました。';
        }
    } catch (Exception $e) {
        $error = 'タイトル更新中にエラーが発生しました: ' . $e->getMessage();
    }
}
...
<form method="POST" class="mb-3">
    ...
    <div class="mb-3">
        <label class="form-label">記事タイトルを修正</label>
        <input type="text" class="form-control" name="new_title" 
               value="<?= htmlspecialchars($_POST['fetched_title'] ?? $existingRegistrations[0]['title'] ?? '') ?>"
               placeholder="最新タイトルが自動取得されました">
        <div class="form-text">
            <i class="fas fa-sync-alt"></i> 
            <?php if (isset($_POST['update_title'])): ?>
                <span class="text-success">タイトルが更新されました！</span>
            <?php elseif (!empty($_POST['fetched_title'])): ?>
                URLから最新タイトルを自動取得しました。
            <?php else: ?>
                登録済み記事のタイトルを修正できます。
            <?php endif; ?>
        </div>
                            ...
                            <form method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="email" class="form-control" name="email"
                                        placeholder="your@email.com（任意）"
                                        value="<?= htmlspecialchars($_POST['email'] ?? '') ?>">
                                    <div class="form-text">
                                        <i class="fas fa-info-circle"></i> 
                                        認証用（後日メール認証を実装予定）。未入力でも登録可能ですが認証機能は利用できません。
                                    </div>
                                </div>
